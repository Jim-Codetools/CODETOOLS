...
const edgeTypes = document.getElementById('edges');
				...
				<div class="nextTo">
					...
					<div class="nextTo" style='margin-bottom:14px;'>
						...
						<div class="nextTo">
							...
							<div id="airDiv${wallTypeCount}" class="sides" style="width:150px; margin-left:0px;">
...
// script to add slab edge details
...
addEdgeBtn.addEventListener('click', (event) => {
	...
	addEdge();
	console.log("Edge button is working");
});


function addEdge() {
	
	const slabLabels = document.getElementById('edges');
	slabLabels.style.display = "";
	
	if (edgeCount<4){
		
		
		edgeCount++;
		const newEdgeType = document.createElement('div');
		newEdgeType.id = 'edgeType' + edgeCount;
		newEdgeType.innerHTML = `
		
			<div class="nextTo">
			
				<div style="width:50px;"></div>
				<div style=" height:28px; display:flex; width:106px; justify-content:center; align-items:center; margin: 0px 0px 5px 0px;">
						
						<p> Edge type ${edgeCount} - </p>
				</div>
				
				<div class="sides1" id="edgeDiv${edgeCount}">
					<input class="distanceA" type="number" id="edgeLen${edgeCount}" name="edgeLen${edgeCount}" min="1" max="200" step="0.1" onchange="updateRoundedValue('edgeLen${edgeCount}'); calcSlabR()"><span class="units">m</span>
				</div>
				
				<div class="sides1">
					<select class="selA" style = "width:90px;" id="edgeStud${edgeCount}" name="edgeStud${edgeCount}" onchange="calcSlabR()">
				...
				<div class="sides1">
					...
					<input type="checkbox" id="edgeIns${edgeCount}" 
					onchange="calcSlabR()">
				
					<label for="slabEdge">R1.0 Insulation</label>		
					
				</div>
				
				<div class="sides1">
				
					<input type="checkbox" id="mas${edgeCount}" 
					onchange="calcSlabR()">
				
					<label for="slabEdge">Masonry Veneer</label>		
		...
		edgeTypes.appendChild(newEdgeType);
...
roofs.addEventListener('click', (event) => {
	...
	if (target.classList.contains('removeSkylight')) {
		...
		if(skyCount > 0){
			...
			console.log(skyToRemove + ' removed');
...
function lessRoof(){
	...
	if(roofCount > 1){
		...
		console.log(roofToRemove + ' removed');
...
function calcRoofR(roofNumber){
	...
	if(roofOver[roofNumber] === 'yes'){
		...
		if(roofType === 'truss') {
			...
			console.log("this is a trussed roof");
			...
			console.log("this is a raftered roof NOW");
		...
		if (roofType === 'truss'){
			...
			console.log("roof framing ratio " + framingRatio);
			console.log("roof insulation ratio " + insuRatio);
			...
			console.log("rafter check " + rafterSelect.value);
			...
			console.log("rafter width " + rafterWidth);
			console.log("rafter thickness " + rafterThick);
			console.log("rafter spacing " + rafterSpacing);
			console.log("nog spacing " + nogSpacing);
			console.log("insulation R-Value " + insuR);
			
			console.log("Rafter insu ratio" + insuRatio);
			console.log("Rafter frame ratio" + framingRatio);
			...
			console.log("Rafter roof has an R value of " + roofRvalue);	
		
		}
	}
	
	console.log("The R value of this roof is " + roofRvalue);
	console.log("the roof num is " + roofNumber);
	console.log("the roof area is " + roofArea);
	
	houseRoofs[(roofNumber-1)].roofR = Number (roofRvalue);
	houseRoofs[(roofNumber-1)].roofArea = Number (roofArea);
	
	console.log("The R value saved in the array is " + houseRoofs[roofNumber-1].roofR + " And the area is " + houseRoofs[roofNumber-1].roofArea);
    ...
    newWall.innerHTML = `
		...
		<div class="wall">
			...
			<div style="margin-left:156px">
				...
				<button class="removeWindow" style = "margin-left: 11px;" data-wall="${wallCount}" id="lessWin${wallCount}">- Joinery</button>
			</div>
...
function wallArea(wallCount){
	...
	if (byAreaCheck.checked) {
		console.log(`Checkbox with ID 'byArea${wallCount}' is checked.`);
		byArea = true;
	} else {
		console.log(`Checkbox with ID 'byArea${wallCount}' is not checked.`);
		byArea = false;
	}
	
	console.log("checking this " + wallCount + " - " + byArea);
	...
	if(byArea === true) {
		console.log("calc by area");
		console.log(wallDimsIn);
		...
		console.log("calc by dimensions");
	...
	console.log("What is the wall area entered ?" + grossWallArea);
    
    console.log("Wall " + wallCount + " Length - " + wallLength);
	console.log("Wall " + wallCount + " Height - " + wallHeight);
	...
	console.log("This wall area = " + houseWalls[(wallCount-1)].wallArea);
	
	console.log(wallArea + "m²");
...
function updateRoundedValue(inputId) {
    ...
    if (!isNaN(userValue)) {
		// Call the roundOne function to round the number
...
function windowArea(wallNum, windowCount){
	...
	if (byAreaCheck.checked) {
		console.log(`**Checkbox with ID 'byArea ${wallNum} ${windowCount}' is checked.`);
		byArea = true;
		...
		console.log(`**Checkbox with ID 'byArea ${wallNum} ${windowCount}' is not checked.`);
		byArea = false;
	...
	console.log("The opening area has changed " + houseWalls[(wallNum-1)].openings[(windowCount-1)].openingArea + "m²" + " Wall-" + wallNum + " Window-" + windowCount);
	
	console.log("wall number =" + wallNum);
	console.log("Opening R val =" + openR);
	
	console.log("The R-Value saved in the array for this opening is R" + houseWalls[(wallNum-1)].openings[(windowCount-1)].openingR + " Wall-" + wallNum + " Window-" + windowCount);
...
function updateResults(){
	...
	for (let i = 0; i < wallCount; i++){
		...
		console.log("Wall " + (i+1) + " - " + grossWallArea + "m²");
		
		let openingCount = houseWalls[i].openings.length;
		
		console.log("Number of openings " + openingCount);
		...
		for (let x = 0; x < openingCount; x++) {
			...
			console.log("gross open area " + grossOpenArea);
		...
		console.log("wall R value" + wallRVal);
...
function updateRoofResults(){
	...
	console.log("updateRoofResults Called");
	...
	for (let i = 0; i < roofCount; i++){
		...
		console.log("Roof " + (i+1) + " - " + grossRoofArea + "m²");
		
		let skyCount = houseRoofs[i].skylights.length;
		
		console.log("Number of skylights " + skyCount);
		...
		for (let x = 0; x < skyCount; x++) {
			...
			console.log("gross skylight area " + grossSkyArea);
		...
		console.log("Roof R value" + roofRVal);
...
function updateOptions(){
	...
	for (let i = 1; i <= wallCount; i++) {
		...
		console.log("how many = " + optionsNum);
		
		for (let x = optionsNum - 1; x >= 0; x--) {
			selectType.remove(x);
			console.log("option " + x + " removed");
		}
		...
		console.log("Current selection " + currentSelections[(i-1)]);
...
// Create wall 1 when the page loads
addWall();

console.log("whichWallType${1}");

// Create wall type 1 when the page loads
addWallType();

// Create roof 1 when the page loads

addRoof();

// Create floor 1 when the page loads
addFloor();
...
lessWallBtn.addEventListener('click', (event) => {
	event.preventDefault();
	console.log("A wall needs to be removed_______");
	lessWall();
});
...
function lessWall(){
	...
	if(wallCount > 1){
		...
		console.log(wallToRemove + ' removed');
...
building.addEventListener('click', (event) => {
	...
	if (target.classList.contains('addWindow')) {
		...
		console.log("initial opening area = " + opening.openingArea);
		(houseWalls[(wallNum-1)].openings).push(opening);
		console.log("A new opening has been created in this wall in the houseWalls array - the area is =" + houseWalls[(wallNum-1)].openings[(windowCount-1)].openingArea);
...
building.addEventListener('click', (event) => {
	...
	if (target.classList.contains('removeWindow')) {
		...
		console.log('wall number ' + wallNum);
		console.log('how many windows' + windowCount);
		
		
		if(windowCount > 0){
			const windowToRemove = document.getElementById(`window${wallNum}_${windowCount}`);
			
			console.log('window to removed' + windowToRemove);
			
			windows.removeChild(windowToRemove);
			
			console.log(windowToRemove + ' removed');
			...
			if(windowCount == 1){
				...
				console.log("window count is only 1");
...
// Slab area to perimeter ratio and R-Value 
...
let slabOver = 'no';
console.log("this here first " + slabOver);
...
function toggleOver(attribute, checked){
	...
	calcSlabR();
	console.log("this here now" + slabOver);
}
...
function toggleRoofOver(roofCount, checked){
	...
	calcRoofR(roofCount);
	console.log("this here now -----" + roofCount + roofOver[roofCount]);
}
...
function toggleWallOver(wallCount, checked){
	...
	wallArea(wallCount);
	console.log("this here now -----" + wallCount + wallOver[wallCount]);
	
	console.log("AAA This one");
}

function toggleFloorOver(floorCount, checked){
	floorOver[floorCount] = checked ? 'yes' : 'no';
	calcFloorR(floorCount);
	console.log("this here  floor " + roofCount + roofOver[roofCount]);
}
...
function calcSlabR() {
	...
	const underSlabR = (document.getElementById('underSlab')).value;
	
	const underCover = (document.getElementById('cover')).value;
	const underCoverSel = (document.getElementById('coverOp'));
	
	const overrideSelect = (document.getElementById('slabOver'));
	const sTypeSelect = (document.getElementById('slabTypeD'));
	
	const slabFeaturesSelect = (document.getElementById('slabFeat'));
	const featuresSel = (document.getElementById('slabFeatures'));
	...
	console.log("The over ride R " + slabOverR);	
	console.log("is embedded heating on?????", embHeat);
	console.log("checking this right now ===", underSlab);
	
	console.log("checking under insulation", underSlabR, underCover);
	
	
	if (underSlabR == 0){
		underCoverSel.style.display = "none";
	}
	else{
		underCoverSel.style.display = "";
	}

	
	
	// code to hide and reveal different inputs
	if (slabOver == 'yes'){
		overrideSelect.style.display = "";
		sTypeSelect.style.display = "none";
		featuresSel.style.display = "none";
		sPeriSelect.style.display = "none";
		periNoteSelect.style.display = "none";
		edgeDisplay.style.display = "none";
	}
	else {
		overrideSelect.style.display = "none";
		sTypeSelect.style.display = "";
		featuresSel.style.display = "";
		sPeriSelect.style.display = "";
		periNoteSelect.style.display = "";
		edgeDisplay.style.display = "";
	}
	...
	let apRatio = 0;
		...
		if (slabOver == 'yes'){
		...
		else if (apRatio >= 0.6 && apRatio <= 5.0){
			
			let mainTable = fetchTable(slabType, false, false, underSlabR, underCover);
			
			console.log("checking the main table ===", mainTable);
			
			if (apRatio > 0){
				slabRvalue = fetchRVal(mainTable, apRatio, 90);
			}
			
			console.log("checking the main R ===", slabRvalue);
			...
			for (let i = 1; i <= edgeCount; i++){
				const eLength = Number ((document.getElementById(`edgeLen${i}`)).value);
				console.log("checking the edge length === ", eLength, i );
				
				const eWidth = Number ((document.getElementById(`edgeStud${i}`)).value);
				console.log("checking the edge width === ", eWidth, i );
				
				const eInsu = ((document.getElementById(`edgeIns${i}`)).checked);
				console.log("checking the edge insu === ", eInsu, i );
				
				const eMason = ((document.getElementById(`mas${i}`)).checked);
				console.log("checking the edge masonry === ", eMason, i );
				
				let portion = eLength/slabPerimeter;
				remainder = remainder - portion;
				
				console.log("checking the portion === ", portion, i );
				
				let table = fetchTable(slabType, eInsu, eMason, underSlabR, underCover);
				
				console.log('the table is =====', table, i);
				
				let rVal = fetchRVal(table, apRatio, eWidth);
				console.log(i, ' the edge R-value is ===', rVal, '!!!!!!');
				
				denominator = denominator + (portion/rVal);	
				
				console.log(i, ' the denominator now == ', denominator);
				
				totalEdgeLen = totalEdgeLen + eLength;
				
			}
		
		
			denominator = denominator + (remainder/slabRvalue);
			
			console.log ('checking the remainder=====', remainder);
			
			console.log ('checking the denominator=====', denominator);
			
			slabRvalue = 1/denominator;
			
			console.log('the r-value is ',slabRvalue);
			...
			console.log("not running an override");
		...
		if (slabLoss > 0 && slabLoss < 500 && slabArea <= 300) {
			...
			console.log("check this here ---" + typeof slabRvalue);
			
			if(apRatio > 0){
				document.getElementById("slabDets").innerHTML = "A/P Ratio " + apRatio + " | R" + slabRvalue.toFixed(2);
			}
			else{
		...
		const lenWarnDiv = document.getElementById(`lenWarnBr`);
		
		if( slabPerimeter < totalEdgeLen){
			lenWarnDiv.style.display = '';
			document.getElementById("sLenWarn").innerHTML = "WARNING! Total edge length must not exceed perimeter length";
		}
		else{
			lenWarnDiv.style.display = 'none';
			document.getElementById("sLenWarn").innerHTML = "";
		}
...
function fetchTable(slabType, eInsu, eMason, underSlabR, underCover){
	
	let whichTable = '';
	
	if (slabType === "raft"){
				
				if (eInsu === false && eMason === true){
					whichTable = 'tableA';
				}
				
				else if (eInsu === false && eMason === false){
					whichTable = 'tableB';
				}
				
				else if (eInsu === true && eMason === true){
					whichTable = 'tableC';
				}
				
				else if (eInsu === true && eMason === false){
					whichTable = 'tableD';
				}
			...
			else if (slabType === "100mm"){
				
				if (eInsu === false && eMason === true && underSlabR == 0){
					whichTable = 'tableE';
				}
				
				else if (eInsu === false && eMason === false && underSlabR == 0){
					whichTable = 'tableF';
				}
				
				else if (eInsu === true && eMason === true && underSlabR == 0){
					whichTable = 'tableG';
				}
				
				else if (eInsu === true && eMason === false && underSlabR == 0){
					whichTable = 'tableH';
				}
				
				else if (eInsu === false && eMason === true && underSlabR == 1.2 && underCover === 'strip'){
					whichTable = 'tableI';
				}
				
				else if (eInsu === false && eMason === false && underSlabR == 1.2 && underCover === 'strip'){
					whichTable = 'tableJ';
				}
				
				else if (eInsu === false && eMason === true && underSlabR == 2.4 && underCover === 'strip'){
					whichTable = 'tableK';
				}
				
				else if (eInsu === false && eMason === false && underSlabR == 2.4 && underCover === 'strip'){
					whichTable = 'tableL';
				}
				
				else if (eInsu === false && eMason === true && underSlabR == 1.2 && underCover === 'full'){
					whichTable = 'tableM';
				}
				
				else if (eInsu === false && eMason === false && underSlabR == 1.2 && underCover === 'full'){
					whichTable = 'tableN';
				}
				
				else if (eInsu === false && eMason === true && underSlabR == 2.4 && underCover === 'full'){
					whichTable = 'tableO';
				}
				
				
				else if (eInsu === false && eMason === false && underSlabR == 2.4 && underCover === 'full'){
					whichTable = 'tableP';
				}
				
				
				else if (eInsu === true && eMason === true && underSlabR == 1.2 && underCover === 'strip'){
					whichTable = 'tableQ';
				}
				
				
				else if (eInsu === true && eMason === false && underSlabR == 1.2 && underCover === 'strip'){
					whichTable = 'tableR';
				}
				
				
				else if (eInsu === true && eMason === true && underSlabR == 2.4 && underCover === 'strip'){
					whichTable = 'tableS';
				}
				
				
				else if (eInsu === true && eMason === false && underSlabR == 2.4 && underCover === 'strip'){
					whichTable = 'tableT';
				}
				
				
				else if (eInsu === true && eMason === true && underSlabR == 1.2 && underCover === 'full'){
					whichTable = 'tableU';
				}
				
				
				else if (eInsu === true && eMason === false && underSlabR == 1.2 && underCover === 'full'){
					whichTable = 'tableV';
				}
				
				else if (eInsu === true && eMason === true && underSlabR == 2.4 && underCover === 'full'){
					whichTable = 'tableW';
				}
				
				
				else if (eInsu === true && eMason === false && underSlabR == 2.4 && underCover === 'full'){
					whichTable = 'tableX';
				}
...
function calcWallR(wallTypeNumber) {
	...
	let studWidth, studThickness, framingRvalue, framingLayer, cavityLayer, claddingLayer, wallRvalue, airBarR;
	...
	let framingRatio = 0.38;
	let insulationRatio = 0.62;
	...
	if (insulationRvalue > 0){
			//document.getElementById(`wallDets${wallTypeNumber}`).innerHTML = `right now`;
...
function setClimate(){
	...
	if (climateZone === '1'){
		...
		if(embHeat == 'no'){
			slabRefR = 1.5;
			console.log('!!!!!No slab heating');
		}
		else{
			slabRefR = 2.5;
			console.log('!!!!!!slab heating');
		}
...
// Handle the user's agreement
function acceptTerms() {
    var agreeCheckbox = document.getElementById("agree");

    if (agreeCheckbox.checked) {
        closeModal(); // Close the modal if the user agrees
    } else {
...
function fetchRVal(tableName, apRatio, wallType) {
  const table = slabTables[tableName];

  let bestMatch = table[0];

  for (let row of table) {
    if (row[0] > apRatio) break;
    bestMatch = row;
  }

  // pick column based on wall type
  return wallType === 90 ? bestMatch[1] : bestMatch[2];
}


	
	
// Table E1.2.1A concrete raft foundation floors without insulation where the external walls have masonry veneer cladding

const slabTables = {
	
// Table A concrete raft foundation floors without insulation where the external walls have masonry veneer cladding
	...
	// Table B concrete raft foundation floors without insulation where the external walls do not have masonry veneer cladding
	...
	// Table C concrete raft foundation floors with R1.0 vertical edge insulation where the external walls have masonry veneer cladding
	...
	// Table D concrete raft foundation floors with R1.0 vertical edge insulation where the external walls do not have masonry veneer cladding
	...
	// Table E slab-floors without insulation where the external walls have masonry veneer cladding
	...
	// Table F slab-floors without insulation where the external walls do not have masonry veneer cladding
	...
	// Table G slab-floors with R1.0 vertical edge insulation without underslab insulation where the external walls have masonry veneer cladding
	...
	// Table H slab-floors with R1.0 vertical edge insulation without underslab insulation where the external walls do not have masonry veneer cladding
	...
	// Table I slab-floors with a 1.2 m wide strip of R1.2 underslab insulation along the slab perimeter where the external walls have masonry veneer cladding
	...
	// Table J slab-floors with a 1.2 m wide strip of R1.2 underslab insulation along the slab perimeter where the external walls do not have masonry veneer cladding
	...
	// Table K slab-floors with a 1.2 m wide strip of R2.4 underslab insulation along the slab perimeter where the external walls have masonry veneer cladding
	...
	// Table L slab-floors with a 1.2 m wide strip of R2.4 underslab insulation along the slab perimeter where the external walls do not have masonry veneer cladding
	...
	// Table M slab-floors with R1.2 full cover underslab insulation where the external walls have masonry veneer cladding
	...
	// Table N slab-floors with R1.2 full cover underslab insulation where the external walls do not have masonry veneer cladding
	...
	// Table O slab-floors with R2.4 full cover underslab insulation where the external walls have masonry veneer cladding
	...
	// Table P slab-floors with R2.4 full cover underslab insulation where the external walls do not have masonry veneer cladding
	...
	// Table Q slab-floors with R1.0 vertical edge insulation and with a 1.2 m wide strip of R1.2 underslab insulation along the slab perimeter where the external walls have masonry veneer cladding
	...
	// Table R slab-floors with R1.0 vertical edge insulation and with a 1.2 m wide strip of R1.2 underslab insulation along the slab perimeter where the external walls do not have masonry veneer cladding
	...
	// Table S slab-floors with R1.0 vertical edge insulation and with a 1.2 m wide strip of R2.4 underslab insulation along the slab perimeter where the external walls have masonry veneer cladding
	...
	// Table T slab-floors with R1.0 vertical edge insulation and with a 1.2 m wide strip of R2.4 underslab insulation along the slab perimeter where the external walls do not have masonry veneer cladding
	...
	// Table U slab-floors with R1.0 vertical edge insulation and with R1.2 full cover underslab insulation where the external walls have masonry veneer cladding
	...
	// Table V slab-floors with R1.0 vertical edge insulation and with R1.2 full cover underslab insulation where the external walls do not have masonry veneer cladding
	...
	// Table W slab-floors with R1.0 vertical edge insulation and with R2.4 full cover underslab insulation where the external walls have masonry veneer cladding
	...
	// Table X slab-floors with R1.0 vertical edge insulation and with R2.4 full cover underslab insulation where the external walls do not have masonry veneer cladding
